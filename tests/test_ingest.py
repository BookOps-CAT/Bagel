...
from context import (
    form_data_reader, lower_first_letter,
    has_alphanumeric_last, remove_white_space_and_trailing_punctuation,
...
def test_form_data_reader_headings():
    ...
    with open(fh, 'r') as csvfile:
        ...
        assert header[9] == 'Price'
        assert header[10] == 'Other titles'
        assert header[11] == 'Subtitle'
        assert header[12] == 'Authors/designers'
        assert header[13] == 'ISBN'
        assert header[14] == 'UPC'
        assert header[15] == 'Place of publication'
        assert header[16] == 'Publisher'
        assert header[17] == 'Date of publication'
        assert header[18] == 'Description/summary'
        assert header[19] == 'List of components'
        assert header[20] == 'Email Address'
